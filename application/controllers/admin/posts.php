...
class Posts extends Admin_Controller {
    ...
    private function search_where($post='') {
        ...
        if (!empty($post['content'])) {
            ...
            $where .= "AND content LIKE '%{$post['content']}%' ";
        }
    ...
    public function check() {
        ...
        $var['topics'] = $topics;
        $var['page'] = $page_str;
        $this->view('posts_check', $var);
    }

    /**
     * 管理审核帖子
     */
    public function deal_check($action='pass',$post_id='') {
        if(!in_array($action, array('pass','del'))){
            $this->message('参数错误，请指定您的操作类型！', 0);
        }
        //格式化ids并检测，两种方式获取ids，get或者post数组。
        empty($post_id) && $post_id = $this->input->post('topic_id');
        is_string($post_id) && $post_id = array_unique(array_filter(explode(',', $post_id)));
        if(empty($post_id)){
        ...
        if ($this->input->post('submit')) {
            ...
            $this->topic_manage->manage_post($post_id,$action,$post);
            $this->message('操作完成！', 1);
        } else {
            $var['action'] = $action;
            $var['action_url'] = 'index.php/admin/posts/deal_check/';
            $var['topic_id'] = join(',', $post_id);
