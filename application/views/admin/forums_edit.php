...
<form method="POST" action="index.php?admin_setting-base">
  <table class="table fspan">
    ...
    <tr>
      <td><span>版块名称</span>
        <input maxlength="30" class="inp_txt" name="setting[site_name]" type="text" value="{$basecfginfo['site_name']}" /></td>
      <td class="v-b" >&nbsp;</td>
    </tr>
    <tr>
      <td><span>版主列表</span>
        <input maxlength="120" class="inp_txt" name="setting[site_url]" type="text" value="{$basecfginfo['site_url']}" /></td>
      <td class="v-b" ><p>多个版主请用英文的逗号、分号或者空格来分隔。</p></td>
    </tr>
    <tr>
      <td><span>排列顺序</span>
        <input style="width:30px" class="inp_txt" name="setting[site_url]" type="text" value="{$basecfginfo['site_url']}" /></td>
      <td class="v-b" ><p>&nbsp;</p></td>
    </tr>
    <tr>
      <td><span>版块图标</span>
        <input type="file" value="" name="iconnew"></td>
      <td class="v-t" ><p>页面底部可以显示第三方统计</p></td>
    </tr>
    <tr>
      <td><span>版块简介</span>
        <textarea cols="45" name="setting[site_notice]" rows="3" class="textarea">{$setting[site_notice]}</textarea></td>
      <td class="v" ><p>支持HTML代码<br/>
        </p></td>
    </tr>
    <tr>
      <td><span>网站备案信息</span>
        <input maxlength="20" class="inp_txt" name="setting[site_icp]" type="text" value="{$basecfginfo['site_icp']}" /></td>
      <td class="v-b" >如果网站已备案，在此输入，它将显示在页面底部，没有请留空</td>
    </tr>
    <tr>
      <td><span>发帖审核:</span>
        <label>
          <input type="radio"  name="check" value="0" {if $basecfginfo['style_user_select']=='1'}checked{/if}/>
          不审核</label>
        <label>
          <input type="radio"  name="check" value="1" {if $basecfginfo['style_user_select']=='0'}checked{/if}/>
          审核主题</label>
        <label>
          <input type="radio"  name="check" value="2" {if $basecfginfo['style_user_select']=='0'}checked{/if}/>
          审核主题和回复</label></td>
    ...
    <tr>
      <td><span>允许html</span>
        <label>
          <input type="radio"  name="is_html" value="1" {if $basecfginfo['compatible']=='1'}checked{/if}/>
          是</label>
        <label>
          <input type="radio"  name="is_html" value="0" {if $basecfginfo['compatible']=='0'}checked{/if}/>
          否</label></td>
      <td></td>
    </tr>
    <tr>
    ...
    <tr>
      <td><span>title</span>
        <input class="inp_txt" name="setting[site_url]" type="text" value="{$basecfginfo['site_url']}" /></td>
      <td></td>
    </tr>
    <tr>
    ...
    <tr>
      <td><span>允许发布的特殊主题</span>
        <ul>
          <li><input type="checkbox" value="1" name="allow_special[]" class="checkbox">投票主题</li>
          <li><input type="checkbox" value="1" name="allow_special[]" class="checkbox">问答主题</li>
          <li><input type="checkbox" value="1" name="allow_special[]" class="checkbox">活动主题</li>
        </ul></td>
      <td></td>
    </tr>
    <tr>
